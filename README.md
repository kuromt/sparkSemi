...
Array[String]をテキストと見なして、テキスト中の単語と単語の出現頻度のベクトルを返す。
...
決定木やTree Ensembleにカテゴリカルな特徴を扱わせるときに使うことができる。パフォーマンスも改善される。
...
withMeanがtrueならスケールする前にデータを平均に集める。

denseなVectorを出力する。sparseな入力には機能しない（将来的に対応予定）。
